...
static void on_status_check_box_toggled(GtkToggleButton *toggle_button,
                                        gpointer user_data);
...
RPIChannelStatus *new_rpi_channel_status(gint channel_id) {
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(instance->activate_channel_check_box), FALSE);
  ...
  gtk_box_pack_start(GTK_BOX(vbox),
                     GTK_WIDGET(instance->status_channel_vertical_bar), FALSE,
                     FALSE, 0);

  gtk_widget_set_halign(instance->activate_channel_check_box, GTK_ALIGN_CENTER);
  gtk_box_pack_start(GTK_BOX(vbox), instance->activate_channel_check_box, FALSE,
                     FALSE, 0);

  gtk_widget_set_halign(instance->status_channel_label, GTK_ALIGN_CENTER);
  gtk_box_pack_start(GTK_BOX(vbox), instance->status_channel_label, FALSE,
                     FALSE, 0);
...
void rpi_channel_status_set_value(RPIChannelStatus *instance, gint value) {
  if (instance) {
    ...
    if (gtk_toggle_button_get_active(
            GTK_TOGGLE_BUTTON(instance->activate_channel_check_box))) {
