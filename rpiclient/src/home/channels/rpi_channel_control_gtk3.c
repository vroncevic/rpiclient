...
static const gchar *const FAILED_MALLOC_CHANNEL_CONTROL =
    "Failed to allocate memory for channel control.\n";
...
static void on_spinner_value_changed(GtkSpinButton *spin_button,
                                     gpointer user_data);
static void on_channel_check_box_toggled(GtkToggleButton *toggle_button,
                                         gpointer user_data);
...
static void on_scale_value_changed(GtkRange *range, gpointer user_data) {
  ...
  if (instance && !instance->is_updating) {
    ...
    gtk_spin_button_set_value(
        GTK_SPIN_BUTTON(instance->control_channel_spinner_button), val);
    if (gtk_toggle_button_get_active(
            GTK_TOGGLE_BUTTON(instance->control_channel_check_box))) {
...
static void on_channel_check_box_toggled(GtkToggleButton *toggle_button,
  ...
  if (instance) {
    ...
    gtk_widget_set_sensitive(instance->control_channel_spinner_button,
                             is_active);
    if (is_active) {
      gint val = (gint)gtk_range_get_value(
          GTK_RANGE(instance->control_channel_scale));
      rpi_network_send_channel(instance->channel_id, val);
    }
  }
}
...
RPIChannelControl *new_rpi_channel_control(gint channel_id) {
  ...
  if (!instance) {
    g_critical("%s", FAILED_MALLOC_CHANNEL_CONTROL);
    return NULL;
  }
  ...
  gtk_style_context_add_class(gtk_widget_get_style_context(instance->frame),
                              "channel-box");
  ...
  gchar tooltip_text_vbar[16] = {0};
  g_snprintf(tooltip_text_vbar, sizeof(tooltip_text_vbar), "Channel %d",
             channel_id);
  gtk_widget_set_tooltip_text(
      GTK_WIDGET(instance->control_channel_vertical_bar), tooltip_text_vbar);

  instance->control_channel_scale = gtk_scale_new_with_range(
      GTK_ORIENTATION_VERTICAL, MIN_VALUE_SCALE_CHANNEL_CONTROL,
      MAX_VALUE_SCALE_CHANNEL_CONTROL, STEP_VALUE_SCALE_CHANNEL_CONTROL);
  ...
  gchar tooltip_text_scale[20] = {0};
  g_snprintf(tooltip_text_scale, sizeof(tooltip_text_scale), "Set Channel %d",
             channel_id);
  gtk_widget_set_tooltip_text(GTK_WIDGET(instance->control_channel_scale),
                              tooltip_text_scale);
  gtk_range_set_inverted(GTK_RANGE(instance->control_channel_scale), TRUE);
  gtk_scale_set_value_pos(GTK_SCALE(instance->control_channel_scale),
                          GTK_POS_TOP);
  gtk_widget_set_size_request(instance->control_channel_scale,
                              WIDTH_SCALE_CHANNEL_CONTROL,
                              HEIGHT_SCALE_CHANNEL_CONTROL);

  instance->control_channel_spinner_adjustment = GTK_ADJUSTMENT(
      gtk_adjustment_new(VALUE_SPINNER_ADJUSTMENT_CHANNEL_CONTROL,
  ...
  instance->control_channel_spinner_button = gtk_spin_button_new(
      instance->control_channel_spinner_adjustment,
  ...
  gtk_widget_set_size_request(
      GTK_WIDGET(instance->control_channel_spinner_button), 90, 30);

  gchar tooltip_text_spinner[32] = {0};
  g_snprintf(tooltip_text_spinner, sizeof(tooltip_text_spinner),
             "Set level at channel %d", channel_id);
  gtk_widget_set_tooltip_text(
      GTK_WIDGET(instance->control_channel_spinner_button),
      tooltip_text_spinner);

  gchar text_check_box[16] = {0};
  g_snprintf(text_check_box, sizeof(text_check_box), "Channel %d",
             channel_id - 1);
  instance->control_channel_check_box =
      gtk_check_button_new_with_label(text_check_box);
  ...
  gchar tooltip_text_check_box[32] = {0};
  g_snprintf(tooltip_text_check_box, sizeof(tooltip_text_check_box),
             "Activate Channel %d", channel_id - 1);
  gtk_widget_set_tooltip_text(GTK_WIDGET(instance->control_channel_check_box),
                              tooltip_text_check_box);
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(instance->control_channel_check_box), FALSE);
  ...
  g_signal_connect(G_OBJECT(instance->control_channel_check_box), "toggled",
                   G_CALLBACK(on_channel_check_box_toggled), instance);
  g_signal_connect(G_OBJECT(instance->control_channel_scale), "value-changed",
                   G_CALLBACK(on_scale_value_changed), instance);
  g_signal_connect(G_OBJECT(instance->control_channel_spinner_button),
                   "value-changed", G_CALLBACK(on_spinner_value_changed),
                   instance);
  ...
  gtk_box_pack_start(GTK_BOX(hbox),
                     GTK_WIDGET(instance->control_channel_vertical_bar), FALSE,
                     FALSE, 0);
  gtk_box_pack_start(GTK_BOX(hbox), GTK_WIDGET(instance->control_channel_scale),
                     FALSE, FALSE, 0);
  gtk_box_pack_start(GTK_BOX(vbox), hbox, FALSE, FALSE, 0);

  gtk_widget_set_halign(instance->control_channel_check_box, GTK_ALIGN_CENTER);
  gtk_box_pack_start(GTK_BOX(vbox), instance->control_channel_check_box, FALSE,
                     FALSE, 0);

  gtk_widget_set_halign(instance->control_channel_spinner_button,
                        GTK_ALIGN_CENTER);
  gtk_box_pack_start(GTK_BOX(vbox), instance->control_channel_spinner_button,
                     FALSE, FALSE, 0);
...
void destroy_rpi_channel_control(RPIChannelControl *instance) {
  if (instance) {
    if (GTK_IS_FRAME(instance->frame)) {
      gtk_widget_destroy(instance->frame);
      instance->frame = NULL;
    }
