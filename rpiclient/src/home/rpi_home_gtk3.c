...
static const gchar *const FAILED_MALLOC_RPI_HOME =
    "Failed to allocate memory for home.\n";
...
static const gchar *const FAILED_PIXBUF_RPI_HOME =
    "Failed to create pixbuf from home icon.\n";
...
RPIHome *new_rpi_home(void) {
  ...
  gtk_box_pack_start(GTK_BOX(instance->vbox),
                     GTK_WIDGET(get_menu_bar_from_rpi_menu(instance->menu_bar)),
                     FALSE, FALSE, 3);
  gtk_box_pack_start(
      GTK_BOX(instance->vbox),
      GTK_WIDGET(get_frame_from_rpi_home_frame(instance->frame_home)), FALSE,
      FALSE, 0);
