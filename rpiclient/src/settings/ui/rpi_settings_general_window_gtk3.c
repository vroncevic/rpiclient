...
/*
 * rpi_settings_general_window_gtk3.c
...
static void on_button_ok_clicked(GtkWidget *widget,
                                 SettingsGeneralWindow *instance);
...
SettingsGeneralWindow *new_settings_general_window(void) {
  ...
  instance->vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL,
                               SPACING_VBOX_SETTINGS_GENERAL_WINDOW);
  ...
  gtk_grid_set_row_spacing(GTK_GRID(instance->grid),
                           TABLE_ROW_SPACINGS_TABLE_SETTINGS_GENERAL_WINDOW);
  gtk_grid_set_column_spacing(GTK_GRID(instance->grid),
  ...
  gtk_frame_set_shadow_type(GTK_FRAME(instance->frame_control_exit),
                            GTK_SHADOW_IN);
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(instance->check_button_control_exit), is_exit_enabled);
  gtk_container_add(GTK_CONTAINER(instance->frame_control_exit),
                    GTK_WIDGET(instance->check_button_control_exit));

  gtk_grid_attach(GTK_GRID(instance->grid),
                  GTK_WIDGET(instance->frame_control_exit), 0, 0, 1, 1);
...
static void on_button_ok_clicked(GtkWidget *widget,
                                 SettingsGeneralWindow *instance) {
  (void)widget;
  const gboolean no_exit_state = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(instance->check_button_control_exit));
