...
/*
 * rpi_settings_network_window_gtk3.c
...
static const gchar *TEXT_FRAME_ENTRY_PORT_SETTINGS_NETWORK_WINDOW =
    "Server Port Number";
...
static void on_button_ok_clicked(GtkWidget *widget,
                                 SettingsNetworkWindow *instance);
static void on_button_cancel_clicked(GtkWidget *widget,
                                     SettingsNetworkWindow *instance);
...
SettingsNetworkWindow *new_settings_network_window(void) {
  ...
  instance->vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL,
                               SPACING_VBOX_SETTINGS_NETWORK_WINDOW);
  ...
  gtk_grid_set_row_spacing(GTK_GRID(instance->table),
                           TABLE_ROW_SPACINGS_TABLE_SETTINGS_NETWORK_WINDOW);
  gtk_grid_set_column_spacing(GTK_GRID(instance->table),
  ...
  gtk_frame_set_shadow_type(GTK_FRAME(instance->frame_control_prompt),
                            GTK_SHADOW_IN);
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(instance->check_button_control_prompt),
      is_prompt_enabled);
  gtk_container_add(GTK_CONTAINER(instance->frame_control_prompt),
                    GTK_WIDGET(instance->check_button_control_prompt));
  gtk_grid_attach(GTK_GRID(instance->table),
                  GTK_WIDGET(instance->frame_control_prompt), 0, 0, 1, 1);
  ...
  gtk_frame_set_shadow_type(GTK_FRAME(instance->frame_entry_address),
                            GTK_SHADOW_IN);
  ...
  gchar *ip_address =
      get_server_ip_address_from_settings(instance->settings);
  instance->cur_pos_address = START_POSITION_ENTRY_ADDRESS_SETTINGS_NETWORK_WINDOW;

  if (ip_address) {
    gtk_editable_insert_text(
        GTK_EDITABLE(instance->entry_address), ip_address,
  ...
  gtk_container_add(GTK_CONTAINER(instance->frame_entry_address),
                    GTK_WIDGET(instance->entry_address));
  gtk_grid_attach(GTK_GRID(instance->table),
                  GTK_WIDGET(instance->frame_entry_address), 0, 1, 1, 1);
  ...
  gtk_frame_set_shadow_type(GTK_FRAME(instance->frame_entry_port),
                            GTK_SHADOW_IN);
  ...
  gchar *port_number =
      get_server_port_number_from_settings(instance->settings);
  instance->cur_pos_port = START_POSITION_ENTRY_PORT_SETTINGS_NETWORK_WINDOW;

  if (port_number) {
    gtk_editable_insert_text(
        GTK_EDITABLE(instance->entry_port), port_number,
  ...
  gtk_container_add(GTK_CONTAINER(instance->frame_entry_port),
                    GTK_WIDGET(instance->entry_port));
  gtk_grid_attach(GTK_GRID(instance->table),
                  GTK_WIDGET(instance->frame_entry_port), 0, 2, 1, 1);
  instance->hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL,
                               SPACING_HBOX_SETTINGS_NETWORK_WINDOW);
  ...
  gtk_widget_set_size_request(GTK_WIDGET(instance->button_ok),
                              WIDTH_BUTTON_HBOX_SETTINGS_NETWORK_WINDOW,
                              HEIGHT_BUTTON_HBOX_SETTINGS_NETWORK_WINDOW);
  gtk_widget_set_size_request(GTK_WIDGET(instance->button_cancel),
                              WIDTH_BUTTON_HBOX_SETTINGS_NETWORK_WINDOW,
                              HEIGHT_BUTTON_HBOX_SETTINGS_NETWORK_WINDOW);
  gtk_container_add(GTK_CONTAINER(instance->hbox),
                    GTK_WIDGET(instance->button_ok));
  gtk_container_add(GTK_CONTAINER(instance->hbox),
                    GTK_WIDGET(instance->button_cancel));
  gtk_box_pack_start(GTK_BOX(instance->vbox), GTK_WIDGET(instance->hbox), FALSE,
                     FALSE, 0);
  g_signal_connect(G_OBJECT(instance->window), "destroy",
                   G_CALLBACK(on_window_destroy), instance);
  g_signal_connect(G_OBJECT(instance->button_ok), "clicked",
                   G_CALLBACK(on_button_ok_clicked), instance);
  g_signal_connect(G_OBJECT(instance->button_cancel), "clicked",
...
void show_settings_network_window(SettingsNetworkWindow *instance) {
  if (instance) {
    ...
    gboolean is_window_hidden =
        !gtk_widget_get_visible(GTK_WIDGET(instance->window));
...
static void on_button_ok_clicked(GtkWidget *widget,
                                 SettingsNetworkWindow *instance) {
  (void)widget;
  const gboolean no_prompt_state = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(instance->check_button_control_prompt));
  set_prompt_enabled_settings(instance->settings, no_prompt_state);
  const gchar *ip_address =
      gtk_entry_get_text(GTK_ENTRY(instance->entry_address));
  set_server_ip_address_settings(instance->settings, ip_address);
  const gchar *port_number =
      gtk_entry_get_text(GTK_ENTRY(instance->entry_port));
