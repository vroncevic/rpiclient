...
//////////////////////////////////////////////////////////////////////////////
/// @brief Help slider complex widget for GTK3
struct _ImageSlider {
  GtkWidget *vbox;
...
ImageSlider *new_image_slider(void) {
  ...
  gtk_container_set_border_width(GTK_CONTAINER(instance->vbox), 10);

  instance->card_frame = gtk_frame_new(NULL);
  gtk_frame_set_shadow_type(GTK_FRAME(instance->card_frame), GTK_SHADOW_IN);

  GtkWidget *card_box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 8);
  gtk_container_set_border_width(GTK_CONTAINER(card_box), 12);

  instance->label_title = gtk_label_new(NULL);
  gtk_label_set_xalign(GTK_LABEL(instance->label_title), 0.0);
  gtk_box_pack_start(GTK_BOX(card_box), instance->label_title, FALSE, FALSE, 0);

  instance->label_body = gtk_label_new(NULL);
  gtk_label_set_use_markup(GTK_LABEL(instance->label_body), TRUE);
  gtk_label_set_line_wrap(GTK_LABEL(instance->label_body), TRUE);
  ...
  gtk_box_pack_start(GTK_BOX(card_box), instance->label_body, TRUE, TRUE, 0);

  gtk_container_add(GTK_CONTAINER(instance->card_frame), card_box);
  gtk_box_pack_start(GTK_BOX(instance->vbox), instance->card_frame, TRUE, TRUE, 0);
  ...
  gtk_widget_set_size_request(instance->button_prev, 85, 30);

  instance->label_step = gtk_label_new("1 / 5");
  gtk_widget_set_size_request(instance->label_step, 60, 30);

  instance->button_next = gtk_button_new_with_label("Next »");
  gtk_widget_set_size_request(instance->button_next, 85, 30);
  ...
  gtk_box_pack_start(GTK_BOX(instance->hbox_nav), instance->button_prev, FALSE,
                     FALSE, 0);
  gtk_box_pack_start(GTK_BOX(instance->hbox_nav), instance->label_step, FALSE,
                     FALSE, 0);
  gtk_box_pack_start(GTK_BOX(instance->hbox_nav), instance->button_next, FALSE,
                     FALSE, 0);

  gtk_box_pack_start(GTK_BOX(instance->vbox), instance->hbox_nav, FALSE, FALSE, 0);
